...
@media screen and (max-width: 600px) {
  .col-25, .col-75, input[type=submit] {
    ...
    <body>
        ...
        <jsp:setProperty name="snr" property="name"/>
        <jsp:setProperty name="snr" property="password"/>
          
        <%-- Display the form data --%>
        The Details Entered Are as Under<br/>
        <p>User Name : <jsp:getProperty name="snr" property="name"/></p>
        <p>Password : <jsp:getProperty name="snr" property="password"/></p>
          
        <%-- Validate the user using the validate() of 
             ValidateUser.java class
        --%>
        <%if(snr.validate("aman", "dummy")){%>
            Welcome AMAN !<br/>
           <% response.sendRedirect("serstory3.1.jsp");%>
        <%}else{%>
            Error! Invalid User Name (or) Incorrect Password<br/>
